...
public class SuffixTree {
    ...
    public void insert (String word) {
        ...
        optimize(root.children);
    }

    private void optimize(HashMap <String, TNode> map) {
        TNode t;
        String [] array;
        TNode toRemove = new TNode("Remove");
        String child;
        TNode test;
        ...
        for (String name: map.keySet()) {
            //if (name == null) break;
            if (map.get(name).children.size() == 1) {
                array = map.get(name).children.keySet().toArray(new String[2]);
                //map.get(name).children.entrySet().toArray();
                child = array[0];
                /*try {
                    child = map.get(name).children.entrySet().iterator().next().getKey();
                } catch (ConcurrentModificationException ex) {
                    break;
                }*/
                System.out.println(child);
                //t = new TNode();
                t = map.get(name).children.get(child);
                System.out.println(t);
                map.put(child, t);
                map.replace(child, map.get(child), toRemove);
            }
            //optimize(map.get(name).children);
        }
        for (String name : map.keySet()) {
            if (map.get(name).data.equals("Remove")) map.remove(map.get(name));
        }
